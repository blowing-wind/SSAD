...
def main():
    ...
    for epoch in range(cfg.TRAIN.BEGIN_EPOCH, cfg.TRAIN.END_EPOCH+1):
        loss_train, cls_loss_ab_record_avg, loc_loss_ab_record_avg = train(cfg, train_loader, model, optimizer)
        print('epoch %d: Train loss: %.4f cls_loss_ab: %.4f, loc_loss_ab: %.4f' % (
            epoch, loss_train, cls_loss_ab_record_avg, loc_loss_ab_record_avg))
        with open(os.path.join(cfg.BASIC.ROOT_DIR, cfg.TRAIN.LOG_FILE), 'a') as f:
            f.write("epoch %d, Train loss: %.4f, cls_loss_ab: %.4f, loc_loss_ab: %.4f\n" % (
                epoch, loss_train, cls_loss_ab_record_avg, loc_loss_ab_record_avg))
