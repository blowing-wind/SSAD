...
def save_model(cfg, epoch, model, optimizer, best=False):
    state = {'epoch': epoch,
             'model': model.state_dict(),
             'optimizer': optimizer.state_dict()}
    save_file = os.path.join(cfg.BASIC.ROOT_DIR, cfg.TRAIN.MODEL_DIR,
                             'Best_model.pth' if best else 'model_'+str(epoch).zfill(2)+'.pth')
