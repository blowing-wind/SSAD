...
def anchor_box_adjust(cfg, anchors, layer):
    ...
    '''
    depth = cfg.MODEL.TEMPORAL_LENGTH[layer]
    # dboxes_w, dboxes_x: ti*n_box. range: (0,1)
    dboxes_w, dboxes_x = default_box(depth, 1.0/depth, cfg.MODEL.ASPECT_RATIOS)
    # temporal overlap
...
def anchor_bboxes_encode(cfg, anchors, glabels, gbboxes, g_action_nums, layer):
    ...
    anchors_class = anchors[:, :, :cfg.DATASET.NUM_CLASSES]
    # dboxes_w, dboxes_x: ti*n_box. range: (0,1)
    dboxes_w, dboxes_x = default_box(temporal_length, 1.0 / temporal_length, cfg.MODEL.ASPECT_RATIOS)
