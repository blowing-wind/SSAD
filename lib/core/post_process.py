...
def final_result_process(out_df, epoch, cfg, flag):
    ...
    for video_name in video_name_list:
        if flag == 0:
            df_ab, df_af = out_df
            tmpdf_ab = df_ab.loc[df_ab.video_name == video_name]
            tmpdf_af = df_af.loc[df_af.video_name == video_name]
            tmpdf = pd.concat([tmpdf_ab, tmpdf_af], sort=True)
        elif flag == 1:
            tmpdf = df_ab.loc[df_ab.video_name == video_name]
        else:
            tmpdf = df_af.loc[df_af.video_name == video_name]

        # assign cliffDiving instance as diving too
        type_set = list(set(tmpdf.cate_idx.values[:]))
        if cfg.TEST.CATE_IDX_OCC in type_set:
            cliff_diving_df = tmpdf.loc[tmpdf.cate_idx == cfg.TEST.CATE_IDX_OCC]
        ...
        # for anet, start and end are in ratio, on need to divide by fps
        fps = get_video_fps(video_name, cfg) if cfg.DATASET.NAME == 'THUMOS14' else 1
