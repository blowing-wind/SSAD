...
class Extractor():
    ...
    def id_and_name(self):
        ...
        if self.use_ambiguous:
            class_id['Ambiguous'] = self.num_class + 1  # wrong!!!
        return class_id
...
if __name__ == '__main__':
    # split in {'val', 'test'}
    split = 'val'
    ann_dir = '/home/0510038/cxl/dataset/thumos14/annotation_val'
    vid_data_dir = '/home/0510038/zyd/data/thumos14/thumos14_val'
    ...
    vid_names_24fps = ['video_validation_0000411']
    # vid_names_25fps = ['video_test_0000950', 'video_test_0001058', 'video_test_0001195', 'video_test_0001255', 'video_test_0001459']
    # vid_names_24fps = ['video_test_0001207']
    dict_cls_type = dict(zip(class_list, type_list))
    # directory to .mp4 files

    extractor = Extractor(split, ann_dir, use_ambiguous=False)
    ...
    for vid_name, vid_anns in gt_segment.items():
        ...
        if vid_name in vid_names_25fps:
            fps = 25
        elif vid_name in vid_names_24fps:
            fps = 24
        else:
            fps = 30
        for ann in vid_anns:
            ...
            assert (len(frames_set) - 1) % 3 == 0
            frame_num.append(int((len(frames_set) - 1) / 3))
    ...
    df.to_csv('val_annotation.csv', encoding='utf-8', index=False)
